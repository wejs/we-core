...
acl.canPolicy = function userCanPolicy(req, callback) {
  ...
  acl.can(permissionName, req.user, record, function (err, can) {
    ...
    if (!can) {
      if (!req.user) {
        req._sails.log.info('ACL:canPolicy: forbidden for unAuthenticated user:', permissionName);
      } else {
        req._sails.log.info('ACL:canPolicy: forbidden for user id: ', req.user.id, req.user.username, permissionName);
      }
    }
