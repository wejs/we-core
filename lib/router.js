...
router.bindResource = function bindResource(opts) {
  ...
  if (opts.namespage) opts.rootRoute = opts.namespage + opts.rootRoute;
  ...
  router.bindRoute(we, 'get '+opts.itemRoute, {
    ...
    name: opts.name + '.findOne',
    action: 'findOne',
    controller: cfg.controller,
    model: cfg.model,
    template: opts.name + '/findOne',
...
router.contextLoader = function contextLoader(req, res, next) {
  ...
  // set redirectTo
  var redirectTo = we.utils.getRedirectUrl(req, res);
