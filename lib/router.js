...
router.contextLoader = function contextLoader(req, res, next) {
  ...
  // set model class
  req.context.Model = req.we.db.models[req.context.model];
  // run model context loader if exists
  if (typeof req.we.db.models[req.context.model].contextLoader == 'function' ) {
    req.we.db.models[req.context.model].contextLoader(req, res, function afterRunModelContextLoader(err) {
