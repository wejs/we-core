...
router.contextLoader = function contextLoader(req, res, next) {
  ...
  var we = req.getWe();
  // set model class
  req.context.Model = we.db.models[req.context.model];
  // run model context loader if exists
  if (typeof we.db.models[req.context.model].contextLoader == 'function' ) {
    we.db.models[req.context.model].contextLoader(req, res, function afterRunModelContextLoader(err) {
