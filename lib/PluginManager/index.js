...
function PluginManager(we) {
  ...
  nodeModulesPath = _path2.default.resolve(projectPath, 'node_modules');
...
PluginManager.prototype.getPluginsList = function getPluginsList(we, done) {
  ...
  _fs2.default.readdir(nodeModulesPath, function (err, folders) {
...
PluginManager.prototype.loadPlugins = function loadPlugins(we, done) {
  // only load one time
  if (!_lodash2.default.isEmpty(this.plugins)) return this.plugins;
  ...
  this.getPluginsList(we, function (err, folders) {
    ...
    for (var i = 0; i < files.length; i++) {
      ...
      // get full path
      pluginPath = _path2.default.resolve(nodeModulesPath, npmModuleName);
      // check if is plugin
      if (pluginManager.isPlugin(pluginPath)) {
        ...
        pluginFile = _path2.default.resolve(pluginPath, 'plugin.js');
...
PluginManager.prototype.getPluginInstallScript = function getPluginInstallScript(name) {
  ...
  if (name == 'project') {
    ...
    pluginFolder = _path2.default.resolve(nodeModulesPath, name);
  }
