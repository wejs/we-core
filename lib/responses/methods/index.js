...
module.exports = {
  ...
  deleted: function deletedResponse() {
    ...
    we.hooks.trigger('we:before:send:deletedResponse', {
      ...
      if (req.accepts('html')) {
        if (res.locals.redirectTo && we.router.urlTo(res.locals.model + '.findOne', req.paramsArray) != res.locals.redirectTo) {
          return res.redirect(res.locals.redirectTo);
        } else {
  ...
  queryError: function queryError(err) {
    ...
    if (err) {
      ...
      if (req.accepts('html') && err.name === 'SequelizeValidationError') {
        // query validation error ...
        res.locals.validationError = {};

        err.errors.forEach(function (err) {
          if (!res.locals.validationError[err.path]) res.locals.validationError[err.path] = [];
