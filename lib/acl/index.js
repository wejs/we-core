...
acl.can = function userCan(permissionName, roleNames, callback) {
  ...
  for (var i = roleNames.length - 1; i >= 0; i--) {
    if (!acl.roles[roleNames]) continue; // role now exists
    if (acl.roles[roleNames].havePermission(permissionName)) {
      can = true
      break;
    }
  }
    ...
    acl.can(res.locals.permission, req.userRoleNames, function (err, can) {
      ...
      if (!can) {
        if (!req.user) {
          ...
          we.log.info('ACL:canMiddleware: forbidden for user id: ', req.user.id, req.user.username, res.locals.permission);
        }
...
acl.loadUserContextRoles = function loadUserContextRoles(req, res, cb) {
  ...
  req.user.getRoles().then(function(roles) {
    req.userRoleNames = roles.map(function(r) {
      return r.name;
    });
