...
acl.can = function userCan(permissionName, user, record, callback) {
  ...
  if ( roles.indexOf('administrator') > -1 ) return callback(null, true);
...
acl.getUserContext = function(user, record) {
  ...
  if (!user) {
    ...
    // - authenticated roles
    if ( user.roles ) {
      context.roles = user.roles.map(function (role) {
        if ( _.isObject(role) ) {
          return role.name;
        } else if ( _.isString(role) ) {
          return role;
        } else {
          console.log('Array user.role contains typeof unknown to acl custom system, typeof === ', typeof(role))
          return role;
        }
      })
    }
...
acl.loadAllPermissions = function loadAllPermissionsFromDB(we, cb) {
  return we.db.models.permission.findAll()
...
acl.fetchAllActionPermissions = function(we, cb) {
  acl.getAllActionPermisons(we, function (err, permissions) {
    if(err) return cb(err);
    ...
    async.each(permissionNames, function(name, next) {
      we.db.models.permission.find({
        ...
        if (result && result.id) {
          ...
          if (!we.acl.permissions[ name ]) we.acl.permissions[ name ] = result;
...
acl.deleteRole = function deleteRole(we, roleId, cb) {
  ...
  return we.db.models.role.find(roleId)
  .done(function (err, role) {
    if (err) return cb(err);

    role.destroy().done(function (err) {
      if (err) return cb(err);
 ...
 * @param {Function} cb             callback
 */
acl.addRoleToPermission = function addRoleToPermission(we, roleName, permissionName, cb) {

  var permission;
  var role;

  async.series([
    function loadRole(done) {
      if (_.isObject( roleName) ) {
        role = roleName;
        return done();
      }
    ...
    function loadPermission(done) {
      if (_.isObject( permissionName) ) {
    ...
    permission.addRole(role).done(function (err, addedRole) {
      if (err) return cb(err);
      if (addedRole) permission.roles.push(addedRole);

      // update permission cache
      we.acl.permissions[ permissionName ] = permission;
