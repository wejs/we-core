...
acl.init = function initAcl (we, cb) {
  ...
  acl.setModelDateFieldPermissions(we);
  ...
  async.parallel([
      ...
      acl.loadAllRoles(we, function (err, roles) {
        if (err) {
          if (
            ...
            // running update how have a fix for this error
            return done();
          }

          return done(err);
        }
        acl.roles = we.utils._.merge(we.config.roles, roles);
        done();
      });
    }
  ], function (err) {
    if (err) {
      if (err.original && err.original.code == 'ER_NO_SUCH_TABLE') {
        we.log.warn('Roles table not found, you installed the system with We.js CLI?');
      } else {
        return cb(err);
      }
    }
...
acl.addPermissionToRole = function addPermissionToRole(we, roleName, permissionName, cb) {
  acl.removeOrAddPermission('addPermission', we, roleName, permissionName, cb);
}
...
acl.removePermissionFromRole = function removePermissionFromRole(we, roleName, permissionName, cb) {
  acl.removeOrAddPermission('removePermission', we, roleName, permissionName, cb);
}

/**
 * Remove or create one permission
 * @param  {String}   action         removePermission or create
 * @param {Object}   we
 * @param {String|Object}   roleName or loaded role
 * @param {String|Object}   permissionName or loaded permission
 * @param {Function} cb             callback
 */
acl.removeOrAddPermission = function removeOrAddPermission(action, we, roleName, permissionName, cb) {
  if (!we.acl.roles[roleName]) return cb( new Error('role not found') );
  we.acl.roles[roleName][action](permissionName).then(function() {
