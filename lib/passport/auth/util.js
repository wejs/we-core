...
util.parseToken = function parseToken(req) {
  ...
  if (req.cookies) {
    // get from cookie
    if (req.cookies && req.cookies[we.config.passport.strategies.weOauth2.cookieName]) {
      return req.cookies[we.config.passport.strategies.weOauth2.cookieName];
    } else if ( req.socket && req.socket.handshake && req.socket.handshake.cookie) {
      // is socket.io request
      if(req.socket.handshake.cookie[we.config.passport.strategies.weOauth2.cookieName])
        return req.socket.handshake.cookie[we.config.passport.strategies.weOauth2.cookieName];
    }
  }
...
util.expireToken = function expireOneToken(token, cb) {
  ...
  .done(function queryResult(err,results) {
    if (err) {
      log.error(err);
      return cb(err);
    }
...
util.loadUserAfterTokenMW = function loadUserAfterTokenMW(req, res, next) {
  ...
  db.models.user.find(req.accessToken.userId)
