...
Plugin.prototype.setResource = function(opts) {
  if (!router.resources[opts.name]) router.resources[opts.name] = {};
  _.merge(router.resources[opts.name], opts);
}
...
Plugin.prototype.loadFeatures = function loadFeatures(we, cb) {
  ...
  async.parallel([
    ...
    function loadPluginTemplates (done) {
      self.loadTemplates(we, done);
    }
  ], function(err){
    if (err) return cb(err);

    if (self.layouts) _.merge(we.view.configuration.layouts, self.layouts);
    if (self.helpers) _.merge(we.view.configuration.helpers, self.helpers);
    if (self.widgets) _.merge(we.view.configuration.widgets, self.widgets);
...
Plugin.prototype.loadTemplates = function loadTemplates(we, cb) {
