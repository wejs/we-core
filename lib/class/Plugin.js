...
module.exports = function (we) {
  ...
  /**
   * Set one route in plugin routes,  we need this function for set custom plugin contexts
   *
  ...
  Plugin.prototype.setRoute = function setRoute(routePath, configs) {
    var routeParts = this.we.router.parseRoute(routePath);

    configs.routePath = routePath;
    configs.pluginPath = this.pluginPath;

    this.routes[routeParts.method][routeParts.path] = configs;
  }
  ...
  Plugin.prototype.setWidgets = function() {
    this.we.log.warn('DEPRECATED! Plugin.setWidgets in plugin: ', this.pluginPath);
  }
