...
module.exports = function (we) {
  ...
  Plugin.prototype.loadSearchParsers = function loadSearchParsers (done) {
    let name, parsers = we.router.search.parsers

    fs.readdir(this.searchParsersPath, (e, fileNames) => {
      if (e) {
      ...
      fileNames.forEach(fileName => {
        if (fileName.endsWith('.js')) {
          ...
          parsers[name] = require(path.join(this.searchParsersPath, fileName)).bind({ we: we })
        }
      })
  ...
  Plugin.prototype.loadSearchTargets = function loadSearchTargets (done) {
    let name, targets = we.router.search.targets

    fs.readdir(this.searchTargetsPath, (e, fileNames) => {
      if (e) {
        if (e.code !== 'ENOENT') return done(e)
        return done()
      }
      ...
      fileNames.forEach(fileName => {
        if (fileName.endsWith('.js')) {
          ...
          name = fileName.slice(0, -3)
          // add in targets
          targets[name] = require(path.join(this.searchTargetsPath, fileName)).bind({ we: we })
        }
      })
