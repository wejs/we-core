...
module.exports = function getPluginPrototype(we) {
  ...
  Plugin.prototype.loadFeatures = function loadFeatures(we, cb) {
    ...
    async.parallel([function loadControllesAndModels(done) {
      self.loadControllers(we);
      self.loadClassMethods(we);
      self.loadInstanceMethods(we);
      self.loadModelHooks(we);
      self.loadModels(we);
      self.loadSearchParsers(we);
      self.loadSearchTargets(we);
      done();
  ...
  Plugin.prototype.loadControllers = function loadControllers(we) {
    ...
    var name;
    try {
      fs.readdirSync(this.controllersPath).forEach(function (fileName) {
        if (fileName.endsWith('.js')) {
          ...
          we.controllers[name] = new we.class.Controller(attrs);
        }
      });
    } catch (e) {
    ...
    try {
      fs.readdirSync(this.modelsPath).forEach(function (fileName) {
        if (fileName.endsWith('.js')) {
          ...
          we.db.modelsConfigs[name] = require(path.join(self.modelsPath, fileName))(we);
        } else if (fileName.endsWith('.json')) {
          // json model
          name = fileName.slice(0, -5); //

          we.db.modelsConfigs[name] = we.db.defineModelFromJson(require(path.join(self.modelsPath, fileName)), we);
        }
      });
    } catch (e) {
      if (e.code !== 'ENOENT') throw e;
  ...
  Plugin.prototype.loadModelHooks = function loadModelHooks(we) {
    ...
    try {
      fs.readdirSync(this.modelHooksPath).forEach(function (fileName) {

        if (fileName.endsWith('.js')) {
          name = fileName.slice(0, -3);

          we.db.modelHooks[name] = require(path.join(self.modelHooksPath, fileName)).bind({ we: we });
        }
      });
    } catch (e) {
      if (e.code !== 'ENOENT') throw e;
   ...
   */
  Plugin.prototype.loadInstanceMethods = function loadInstanceMethods(we) {
    var self = this;
    var name;

    try {
      fs.readdirSync(this.modelInstanceMethodsPath).forEach(function (fileName) {

        if (fileName.endsWith('.js')) {
          name = fileName.slice(0, -3);

          we.db.modelInstanceMethods[name] = require(path.join(self.modelInstanceMethodsPath, fileName));
        }
      });
    } catch (e) {
      if (e.code !== 'ENOENT') throw e;
   ...
   */
  Plugin.prototype.loadClassMethods = function loadClassMethods(we) {
    var self = this;
    var name;

    try {
      fs.readdirSync(this.modelClassMethodsPath).forEach(function (fileName) {

        if (fileName.endsWith('.js')) {
          name = fileName.slice(0, -3);

          we.db.modelClassMethods[name] = require(path.join(self.modelClassMethodsPath, fileName));
        }
      });
    } catch (e) {
      if (e.code !== 'ENOENT') throw e;
   ...
   */
  Plugin.prototype.loadSearchParsers = function loadSearchParsers() {
    var name,
        self = this,
        parsers = we.router.search.parsers;

    try {
      fs.readdirSync(this.searchParsersPath).forEach(function (fileName) {

        if (fileName.endsWith('.js')) {
          name = fileName.slice(0, -3);

          parsers[name] = require(path.join(self.searchParsersPath, fileName)).bind({ we: we });
        }
      });
    } catch (e) {
      if (e.code !== 'ENOENT') throw e;
   ...
   */
  Plugin.prototype.loadSearchTargets = function loadSearchTargets() {
    var name,
        self = this,
        targets = we.router.search.targets;

    try {
      fs.readdirSync(this.searchTargetsPath).forEach(function (fileName) {

        if (fileName.endsWith('.js')) {
          name = fileName.slice(0, -3);

          targets[name] = require(path.join(self.searchTargetsPath, fileName)).bind({ we: we });
        }
      });
    } catch (e) {
      if (e.code !== 'ENOENT') throw e;
