...
/**
 * Parse one route and extracts method from url
 *
 * @param  {String} route
 * @return {Object}
 */
Router.prototype.parseRoute = function(route) {
  ...
  var r = route.split(' ');
  if (r.length > 1) {
    routeParts.method = r[0];
    routeParts.path = r[1];
  } else {
    routeParts.method = 'get';
    routeParts.path = r[0];
  }
...
/**
 * [routeSelectorMiddleware description]
 * @param  {[type]}   req  [description]
 * @param  {[type]}   res  [description]
 * @param  {Function} next [description]
 * @return {[type]}        [description]
 */
Router.prototype.routeSelectorMiddleware = function routeSelectorMiddleware(req, res, next) {
  var we = req.we;

  var method = req.method.toLowerCase();

  // method not found
  if (!we.routes[method]) return next();
  if (!we.routes[method][req.path]) return next();

  var config = we.routes[method][req.path]
  ...
  var middlewares = [
    ...
    we.router.parseBody,
    // run context loader
    we.router.contextLoader,
    // run acl middleware
    we.acl.canMiddleware,
    // run update widget middleware
    we.view.updateWidgetMiddleware,
  ];
  ...
  if (
    ...
    return we.log.warn('app.router.bindRoute: Unknow controller or action:', req.path, config);
  }
  ...
  if (config.upload) middlewares.push(uploader(config.upload));

  /**
   * Use this event to change we.js route middlewares
   * @type {Event}
   */
  we.events.emit('router:before:set:controller:middleware', {
    we: we, middlewares: middlewares, config: config
  });
  // bind contoller
  ...
  we.utils.async
  .eachSeries(middlewares, function eachMiddleware (middleware, done){
    middleware.bind({ config: config })(req, res, done);
  }, next);
}
 ...
 */
Router.prototype.saveRouteMap = function saveRouteMap (we, config){
  var mapName = config.name;
  if (!mapName) mapName = config.controller + '.' + config.action;

  if (!this.routeMap[config.method]) this.routeMap[config.method] = {};

  var map = this.parseRouteToMap(config.path);

  // map get routes for use with link-to
  this.routeMap[config.method][mapName] = {
    map: map,
    config: config
  };
  // save map in configs for run params middlewares after route
  config.map = map;
}
