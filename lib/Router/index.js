...
var Router = function routerPrototype(we) {
  ...
  if (we.plugins['we-plugin-acl']) {
    we.events.on('we-core:before:bind:one:resource:route', function(data) {
      // set crud permissions
      this.crudPermissioPrefix.forEach(function(ai){
        if (!we.config.permissions[ai+'_'+data.configuration.model]) {
          we.config.permissions[ai+'_'+data.configuration.model] = {
...
Router.prototype.crudPermissioPrefix = ['find', 'create', 'update', 'delete'];
