...
function Router(we) {
  ...
  router.singleRecordActions = ['findOne', 'update', 'destroy', 'updateAttribute', 'deleteAttribute', 'addRecord', 'removeRecord', 'getRecord', 'edit', 'delete'];
...
Router.prototype.bindRoute = function bindRoute(app, route, config, groupRouter) {
  ...
  if (app.controllers[config.controller] && app.controllers[config.controller][config.action]) {
  ...
  (0, _cors2.default)(config.CORS || app.config.security.CORS),
  // body we.js parser
  ...
  if (!mapName) mapName = config.controller + '.' + config.action;
...
Router.prototype.contextLoader = function contextLoader(req, res, next) {
  ...
  _lodash2.default.merge(res.locals, config);
  // save all params values as array for router.urlTo
  req.paramsArray = _lodash2.default.toArray(req.params);
  // set accept headers based in config.responseType, this overrides req.query.responseType
  if (config.responseType) req.headers.accept = _mime2.default.lookup(config.responseType);
  ...
  hooks.trigger('we:router:request:before:load:context', {
    ...
    router.parseQuery(req, res, function (err, query) {
      ...
      // default template
      if (!res.locals.template) res.locals.template = res.locals.model + '/' + res.locals.action;
      // run model context loader if exists
      if (req.we.db.models[res.locals.model] && typeof req.we.db.models[res.locals.model].contextLoader == 'function') {
...
Router.prototype.contextLoaded = function contextLoaded(err) {
  ...
  this.hooks.trigger('we:router:request:after:load:context', {
    ...
    // preload user roles
    return this.req.user.getRoles().nodeify(function afterGetUserRoles(err, result) {
...
Router.prototype.resourceCacheMiddleware = function resourceCacheMiddleware(req, res, next) {
  ...
  if (!req.we.config.cache.skipResourceCache && req.we.config.cache.resourceCacheActions.indexOf(res.locals.action) > -1 && res.locals.data && res.locals.data.updatedAt) {
...
Router.prototype.parseQueryWhere = function parseQueryWhere(req, res, query) {
  ...
  if (res.locals.routeQuery) _lodash2.default.merge(query.where, res.locals.routeQuery);
  // parse associations
  ...
  if (res.locals.search) {
    for (var sName in res.locals.search) {
      if (req.query[sName] || res.locals.search[sName].runIfNull) {
        this.search.targets[res.locals.search[sName].target.type](sName, res.locals.search[sName], req.query[sName], query, req, res);
      }
    }
  }
};
...
Router.prototype.parseBody = function parseBody(req, res, next) {
  var parsers = req.we.responses.parsers;
...
Router.prototype.addRequestNx1Assocs = function addRequestNx1Assocs(req, res, query) {
  if (res.locals.model && req.we.db.modelsConfigs[res.locals.model] && req.we.db.modelsConfigs[res.locals.model].associations) {
    query.include = [];
...
Router.prototype.isPublicFolder = function isPublicFolder(url) {
  return url.startsWith('/public') || url == '/robots.txt' || url == '/favicon.ico';
};
