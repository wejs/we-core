...
db.connect = function connect() {
  ...
  _.merge(configs, db.defaultModelDefinitionConfigs);
  ...
  for ( var modelName in db.modelsConfigs) {
    ...
    for (var attributeName in db.modelsConfigs[modelName].associations) {
      ...
      try {
        ...
        console.log('Error on setModelAllJoins 2: ', attrConfig.model, db.models[attrConfig.model]);
        throw e;
      }
    }
  }
...
db.defaultModelDefinitionConfigs = {
  define: {
    // table configs
    timestamps: true,
    createdAt:  'createdAt',
