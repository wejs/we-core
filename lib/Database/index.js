...
Database.prototype.connect = function connect() {
  ...
  (0, _lodash.merge)(configs, this.defaultModelDefinitionConfigs);
  ...
  if (configs.uri) {
    return new _sequelize2.default(configs.uri, configs);
  } else {
    return new _sequelize2.default(configs.database, configs.username, configs.password, configs);
  }
};
...
Database.prototype.loadCoreModels = function loadCoreModels(done) {
  ...
  this.models.plugin = this.define('plugin', {
    filename: {
      ...
      type: _sequelize2.default.STRING(1000),
      allowNull: false
    },
    name: {
      comment: 'plugin name',
    ...
    type: {
      type: _sequelize2.default.STRING(12),
      defaultValue: 'plugin',
      allowNull: false
    },
    status: {
    ...
    weight: {
      ...
      type: _sequelize2.default.INTEGER(5),
      defaultValue: 0,
      allowNull: false
    },
    info: {
  ...
  db.models.plugin.sync().then(function () {
    done();
...
Database.prototype.syncAllModels = function syncAllModels(cd, cb) {
  ...
  if (this.env == 'test' || this.env != 'prod' && cd && cd.resetAllData) {
    this.defaultConnection.sync({ force: true }).then(function () {
      cb();
    }).catch(cb);
  } else {
    this.defaultConnection.sync().then(function () {
      cb();
    }).catch(cb);
  }
};
...
Database.prototype.setModelHooks = function setModelHooks() {
  ...
  modelNames.filter(function (mn) {
    ...
    for (hname in hooks) {
      ...
      if ((0, _lodash.isArray)(hooks[hname])) {
        fns = hooks[hname];
      } else {
