...
var view = {
    ...
    we.events.on('we:after:load:plugins', function (we) {
      ...
      for (var i = 0; i < themesConfig.enabled.length; i++) {
        if (we.utils._.isString(i)) {
          ...
          view.themes[name] = new Theme(
            themesConfig.enabled[i].name, we.projectPath, themesConfig.enabled[i]
          );
        }

        view.themes[name].projectThemeName = themesConfig.enabled[i];
      }
  ...
  resolveLayout: function resolveLayout(req, res, next) {
    ...
    if (!res.locals.layoutName || res.locals.layoutName === 'default') {
      if (res.locals.model && theme && theme.layouts && theme.layouts[res.locals.model+'-layout']) {
