...
var router = {
  ...
  initialize: function initialize(we) {
    ...
    router.routers.admin = we.utils.express.Router();
  },
   ...
   * @return {Object|Null} returns the router or null
   */
  RouterFor: function RouterFor(path, config) {
    if (config.pathParts && router.routers[config.pathParts[0]]) {
      return router.routers[config.pathParts[0]];
    } else {
      return null;
    }
  },

  setExpressRouters: function setExpressRouters(we, cb) {
    for(var r in router.routers) {
      we.express.use('/'+r, router.routers[r]);
    }
    cb();
  }
};
...
hooks.on('we:after:resources:bind', router.setExpressRouters);
...
router.bindRoute = function bindRoute(we, route, config, groupRouter) {
  ...
  // path parts slited in /
  config.pathParts = path.slice(1).split('/');

  if (!groupRouter) {
    groupRouter = router.RouterFor(path, config);
    if (groupRouter) {
      // if have a group router update path to remove base path
      path = path.replace('/'+config.pathParts[0] , '');
    } else {
      groupRouter = we.express;
    }
  }
