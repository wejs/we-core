...
router.resourceBinder.bindCreateResource = function bindCreateResource(we, cfg, opts) {
  // deafult breadcrumb for create routes
  var createBreadcrumb = function breadcrumb() {
    return [{
      href: '/',
      text: this.__('Home')
    }, {
      href: router.urlTo(opts.name + '.find', this.req.paramsArray) ,
      text: this.__(opts.name + '.find')
    }, {
      href: this.req.urlBeforeAlias,
      text: this.__(opts.name + '.create')
    }]
  }
  ...
  router.bindRoute(we, 'get '+opts.itemRoute, _.merge({
    ...
    breadcrumb: function breadcrumb() {
      return [{
        ...
        href: this.req.urlBeforeAlias,
        text: we.utils.string(this.title).truncate(25).s
      }]
    }
  }, opts.findOne));
}
...
router.resourceBinder.bindUpdateResource = function bindUpdateResource(we, cfg, opts, Model) {
  // deafult breadcrumb for update routes
...
router.resourceBinder.bindDeleteResource = function bindDeleteResource(we, cfg, opts, Model) {
  ...
  // deafult breadcrumb for delete routes
  var deleteBreadcrumb = function breadcrumb() {
    return [{
      href: '/',
      text: this.__('Home')
    }, {
