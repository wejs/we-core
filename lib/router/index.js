...
hooks.on('we:router:request:after:load:context', function titleMiddleware(data, done) {
  router.title.middleware(data.req, data.res, done);
});

hooks.on('we:router:request:after:load:context', function metatagMiddleware(data, done) {
  // only run on html response
  if (data.res.locals.responseType == 'html') {
