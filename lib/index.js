...
function We(options) {
  ...
  we.bootstrapFunctions = [function loadCoreFeatures(we, next) {
    ...
    we.config = _staticConfig2.default.loadPluginConfigs(we);

    we.hooks.trigger('we:before:load:plugin:features', we, function () {
      _async2.default.eachSeries(we.pluginNames, function (pluginName, next) {
    ...
    we.hooks.trigger('we:models:before:instance', we, function (err) {
      ...
      for (var modelName in we.db.modelsConfigs) {
        var mc = we.db.modelsConfigs[modelName];
        ...
        if (mc.associations) mc.associationNames = Object.keys(mc.associations);

        // define the model
        we.db.models[modelName] = we.db.define(modelName, mc.definition, mc.options);
      }
    ...
    we.db.defaultConnection.sync().then(function afterSyncAllPluginTables() {
      ...
      _async2.default.eachSeries(names, function onEachPlugin(name, nextPlugin) {
        // run install scripts
    ...
    // load express
    we.express = (0, _express2.default)(we);
    ...
    for (var plugin in we.plugins) {
      _lodash2.default.merge(we.routes, we.plugins[plugin].routes);
    }
    // merge project routes
    _lodash2.default.merge(we.routes, we.config.routes);
    next();
  },
  /**
