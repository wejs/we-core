...
var We = function WePrototype(options) {
  ...
  we.class = {
    ...
    Widget: require('./class/Widget.js')(we),
  ...
  we.hooks.on('bootstrap', [
      ...
      we.hooks.trigger('we:models:before:instance', we, function (err) {
        ...
        for ( var modelName in we.db.modelsConfigs) {
          ...
          if (haveAlias) {
            ...
            we.db.models[modelName].addHook('afterCreate', 'createUrlAlias', we.router.alias.afterCreatedRecord);
            we.db.models[modelName].addHook('afterUpdate', 'updateUrlAlias', we.router.alias.afterUpdatedRecord);
            we.db.models[modelName].addHook('afterDestroy', 'destroyUrlAlias', we.router.alias.afterDeleteRecord);
          }
        }
    ...
    function bindRoutes(we, next) {
      ...
      we.hooks.trigger('we:before:routes:bind', we, function beforeRouteBind() {
        // add the route selecotor middleware, is faster than bind all routes
        we.express.use(we.router.routeSelectorMiddleware);
        var method, route;

        for (method in we.routes) {
          for (route in we.routes[method]) {
            // set route and method for route map
            we.routes[method][route].path = route;
            we.routes[method][route].method = method;

            we.router.saveRouteMap(we, we.routes[method][route]);
          }
        }
...
We.prototype.go = function go(cfgs, cb) {
  ...
  this.bootstrap(cfgs, function (err, we) {
    if (err) throw err;
    we.startServer(cb);
  });
}
