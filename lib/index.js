...
function We(options) {
  ...
  switch (we.config.bootstrapMode) {
    ...
    case 'installation':
    case 'update':
      we.hooks.on('bootstrap', [we.bootstrapFunctions.checkDBConnection, we.bootstrapFunctions.loadCoreFeatures, we.bootstrapFunctions.loadPluginFeatures, we.bootstrapFunctions.loadTemplateCache, we.bootstrapFunctions.instantiateModels, we.bootstrapFunctions.syncModels, we.bootstrapFunctions.loadControllers, we.bootstrapFunctions.initI18n, we.bootstrapFunctions.installAndRegisterPlugins]);
      break;
    case 'complete':
    case 'full':
    case 'test':
      // full load, usefull for tests
      we.hooks.on('bootstrap', [we.bootstrapFunctions.checkDBConnection, we.bootstrapFunctions.loadCoreFeatures, we.bootstrapFunctions.loadPluginFeatures, we.bootstrapFunctions.loadTemplateCache, we.bootstrapFunctions.instantiateModels, we.bootstrapFunctions.syncModels, we.bootstrapFunctions.loadControllers, we.bootstrapFunctions.initI18n, we.bootstrapFunctions.installAndRegisterPlugins, we.bootstrapFunctions.setExpressApp, we.bootstrapFunctions.passport, we.bootstrapFunctions.createDefaultFolders, we.bootstrapFunctions.registerAllViewTemplates, we.bootstrapFunctions.mergeRoutes, we.bootstrapFunctions.bindResources, we.bootstrapFunctions.bindRoutes]);
      break;
    default:
