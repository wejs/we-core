...
var _staticConfig = require('./staticConfig');

var _staticConfig2 = _interopRequireDefault(_staticConfig);
...
function We(options) {
  ...
  this.projectPackageJSON = require(_path2.default.resolve(this.projectPath, 'package.json'));

  this.projectConfigFolder = options.projectConfigFolder || _path2.default.join(this.projectPath, 'config');

  // start configs with static configs
  this.config = (0, _staticConfig2.default)(this.projectPath, this);
  // enviroment config prod | dev | test
  we.env = options.env || require('./getEnv.js')();
  // winston logger
  we.log = require('./log')(we);
  // hooks and events
  we.hooks = new _Hooks2.default();

  we.events = new _events2.default();
  we.events.setMaxListeners(70);
  // we.js sanitizer
  we.sanitizer = new _Sanitizer2.default(this);
  // The we.js router
  we.router = new _Router2.default(this);
  // we.js prototypes
  we.class = {
  ...
  we.db = new _Database2.default(this);
  // - init database
  ...
  we.pluginManager = new _PluginManager2.default(this);

  switch (we.config.bootstrapMode) {
    case 'install':
      we.hooks.on('bootstrap', [we.bootstrapFunctions.loadCoreFeatures, we.bootstrapFunctions.loadPluginFeatures, we.bootstrapFunctions.loadTemplateCache, we.bootstrapFunctions.instantiateModels, we.bootstrapFunctions.syncModels, we.bootstrapFunctions.loadControllers, we.bootstrapFunctions.initI18n, we.bootstrapFunctions.installAndRegisterPlugins]);
      break;
    case 'complete':
    case 'full':
    case 'test':
      // full load, usefull for tests
      we.hooks.on('bootstrap', [we.bootstrapFunctions.loadCoreFeatures, we.bootstrapFunctions.loadPluginFeatures, we.bootstrapFunctions.loadTemplateCache, we.bootstrapFunctions.instantiateModels, we.bootstrapFunctions.syncModels, we.bootstrapFunctions.loadControllers, we.bootstrapFunctions.initI18n, we.bootstrapFunctions.installAndRegisterPlugins, we.bootstrapFunctions.setExpressApp, we.bootstrapFunctions.passport, we.bootstrapFunctions.createDefaultFolders, we.bootstrapFunctions.registerAllViewTemplates, we.bootstrapFunctions.mergeRoutes, we.bootstrapFunctions.bindResources, we.bootstrapFunctions.bindRoutes]);
      break;
    default:
      // defaults to load for run
      we.hooks.on('bootstrap', [we.bootstrapFunctions.loadCoreFeatures, we.bootstrapFunctions.loadPluginFeatures, we.bootstrapFunctions.loadTemplateCache, we.bootstrapFunctions.instantiateModels, we.bootstrapFunctions.syncModels, we.bootstrapFunctions.loadControllers, we.bootstrapFunctions.initI18n, we.bootstrapFunctions.setExpressApp, we.bootstrapFunctions.passport, we.bootstrapFunctions.createDefaultFolders, we.bootstrapFunctions.registerAllViewTemplates, we.bootstrapFunctions.mergeRoutes, we.bootstrapFunctions.bindResources, we.bootstrapFunctions.bindRoutes]);
  }
}
...
We.prototype.setConfig = function setConfig(variable, value, cb) {
  var cJSON,
      cFGpath = _path2.default.join(this.projectPath, '/config/configuration.json');

  try {
    cJSON = JSON.parse((0, _fs.readFileSync)(cFGpath));
  } catch (e) {
    if (e.code == 'ENOENT') {
      (0, _fs.writeFileSync)(cFGpath, '{}');
      cJSON = {};
    } else {
      return cb(e);
    }
  ...
  _lodash2.default.set(cJSON, variable, value);

  (0, _fs.writeFile)(cFGpath, JSON.stringify(cJSON, null, 2), cb);
};
...
We.prototype.unSetConfig = function unSetConfig(variable, cb) {
  ...
  _lodash2.default.unset(cJSON, variable);
...
We.prototype.weCorePluginfile = _path2.default.resolve(__dirname, '../') + '/plugin.js';
...
We.prototype.bootstrap = function bootstrap(configOnRun, cb) {
  ...
  if (configOnRun) _lodash2.default.merge(we.config, configOnRun);
...
We.prototype.startServer = function startExpressServer(cb) {
  if (!cb) cb = function cb() {}; // cb is optional
  ...
  we.hooks.trigger('we:server:before:start', we, function afterRunBeforeServerStart(err) {
     ...
     */
    var server = _http2.default.createServer(function onCreateServer(req, res) {
      req.we = we;
