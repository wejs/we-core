...
we.bootstrap = function bootstrap(configOnRun, cb) {
  ...
  async.series([
    ...
    function initializeAliasFeature(next) {
      we.log.verbose('initializeAliasFeature step');
      we.router.alias.initialize(we, next);
    },
    ...
    function bindResources(next) {
      ...
      we.hooks.trigger('we:after:resources:bind', we, function afterBindResources() {
        next();
      });
    }
...
we.startServer = function startExpressServer(cb) {
  we.hooks.trigger('we:server:before:start',we, function (err) {
    ...
    if (we.config.enableUrlAlias) {
      server = http.createServer(we.router.alias.httpHandler);
    } else {
      server = http.createServer(we.express);
    }
