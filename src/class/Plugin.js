...
module.exports = function getPluginPrototype (we) {
  ...
  Plugin.prototype.loadInstanceMethods = function loadInstanceMethods (we) {
    ...
    try {
      fs.readdirSync(this.modelInstanceMethodsPath)
      .forEach(function (fileName) {

        if (fileName.endsWith('.js')) {
          name = fileName.slice(0, -3);

          we.db.modelInstanceMethods[name] =
            require(path.join(self.modelInstanceMethodsPath, fileName));
        }
      });
    } catch(e) {
      if (e.code !== 'ENOENT') throw e;
  ...
  Plugin.prototype.loadClassMethods = function loadClassMethods (we) {
    ...
    try {
      fs.readdirSync(this.modelClassMethodsPath)
      .forEach(function (fileName) {

        if (fileName.endsWith('.js')) {
          name = fileName.slice(0, -3);

          we.db.modelClassMethods[name] =
            require(path.join(self.modelClassMethodsPath, fileName));
        }
      });
    } catch(e) {
      if (e.code !== 'ENOENT') throw e;
  ...
  Plugin.prototype.loadSearchParsers = function loadSearchParsers () {
    var name, self = this, parsers = we.router.search.parsers

    try {
      fs.readdirSync(this.searchParsersPath)
      .forEach(function (fileName) {
  ...
  Plugin.prototype.loadSearchTargets = function loadSearchTargets () {
    var name, self = this, targets = we.router.search.targets

    try {
      fs.readdirSync(this.searchTargetsPath)
      .forEach(function (fileName) {
